                ...
                <tbody>
                    {bills.map((bill) => (
                        <tr key={bill.id}>
                            ...
                            <td className={bill.status === "PAID" ? "bg-green-500 text-white" : "bg-gray-500 text-white"}>{bill.status}</td>
                            <td>{bill.customer ? `${bill.customer.firstName} ${bill.customer.lastName}` : "Over The Counter"}</td>
                        </tr>
                    ))}
                </tbody>
            </table>
